...
class AndroidEnvironment {
  ...
  Future<Map<String, String>> buildEnvironment() async {
    ...
    environment.addAll(_gstreamerEnvironment());
    return environment;
  }

  Map<String, String> _gstreamerEnvironment() {
    final root = Platform.environment['GSTREAMER_ROOT_ANDROID'];
    if (root == null || root.isEmpty) {
      return {};
    }

    final abiRoot = path.join(root, _gstreamerAndroidAbi(target));
    final prefix =
        Directory(path.join(abiRoot, 'lib', 'pkgconfig')).existsSync()
            ? abiRoot
            : root;

    final pkgConfigPath = [
      path.join(prefix, 'lib', 'pkgconfig'),
      path.join(prefix, 'lib', 'gstreamer-1.0', 'pkgconfig'),
      path.join(prefix, 'lib', 'gio', 'modules', 'pkgconfig'),
    ];

    final gstreamerPc =
        File(path.join(pkgConfigPath.first, 'gstreamer-1.0.pc'));
    if (!gstreamerPc.existsSync()) {
      throw Exception(
        'GStreamer Android SDK for ${target.android} was not found at $prefix. '
        'Set GSTREAMER_ROOT_ANDROID to the extracted universal SDK root.',
      );
    }

    final existingPkgConfigPath = Platform.environment['PKG_CONFIG_PATH'];
    final effectivePkgConfigPath = [
      ...pkgConfigPath,
      if (existingPkgConfigPath != null && existingPkgConfigPath.isNotEmpty)
        existingPkgConfigPath,
    ].join(Platform.isWindows ? ';' : ':');

    return {
      'GSTREAMER_ROOT_ANDROID': root,
      'GSTREAMER_ANDROID_PREFIX': prefix,
      'GSTREAMER_ANDROID_ABI': _gstreamerAndroidAbi(target),
      'PKG_CONFIG_ALLOW_CROSS': '1',
      'PKG_CONFIG_PATH': effectivePkgConfigPath,
      'PKG_CONFIG_LIBDIR': pkgConfigPath.join(Platform.isWindows ? ';' : ':'),
      'SYSTEM_DEPS_LINK': 'static',
    };
  }

  String _gstreamerAndroidAbi(Target target) {
    return switch (target.android) {
      'armeabi-v7a' => 'armv7',
      'arm64-v8a' => 'arm64',
      'x86' => 'x86',
      'x86_64' => 'x86_64',
      _ => throw Exception('Unsupported Android target: ${target.android}'),
    };
  }
